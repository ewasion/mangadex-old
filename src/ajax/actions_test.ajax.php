	...
	*/
	...
	case "login":
		...
		if ($count) {
			...
			$details = "Incorrect username: $username";
	...
	case "signup":		
		...
		if ($sign_up_test) {
			...
			$db->query("INSERT INTO mangadex_users (user_id, username, password, password_smf, token, level_id, email, language, display_lang_id, default_lang_ids, style, joined_timestamp, last_seen_timestamp, avatar, cip, time_offset, activation_key, activated, user_website, user_description, upload_group_id, upload_lang_id, read_announcement, user_views, hentai_mode, swipe_direction, swipe_sensitivity, reader_mode, reader_click) 
				VALUES (NULL, '$username', '$password_hash', '$password_smf', '$token', 2, '$email1', 1, 1, '', 1, $timestamp, $timestamp, NULL, '$ip', '0', '$activation_key', 0, '', '', 0, 0, 0, 0, 0, 1, 100, 0, 1);" );
	...
	*/
	case "chapter_delete":
		...
		if ($chapter->exists) {
			...
			if ($user->level_id >= 10 || $chapter->user_id == $user->user_id || $group->group_leader_id == $user->user_id) {
				$db->query(" INSERT INTO mangadex_chapters_deleted SELECT * FROM mangadex_chapters WHERE chapter_id = $id; "); //copy row into deleted table
				
				$db->query(" DELETE FROM mangadex_chapters WHERE chapter_id = $id LIMIT 1; "); //delete from actual table
	...
	case "chapter_upload":
		...
		if (!$error) {
			...
			$db->query("INSERT INTO mangadex_chapters (chapter_id, chapter_hash, manga_id, volume, chapter, title, upload_timestamp, user_id, chapter_views, lang_id, authorised, group_id, group_id_2, group_id_3, server, page_order) 
			VALUES (NULL, '$chapter_hash', $manga_id, '$volume_number', '$chapter_number', '$chapter_name', $upload_timestamp, $user->user_id, 0, $lang_id, 0, $group_id, 0, 0, 0, '$page_order'); ");
		...
		if ($manga->exists) {
			...
			if (!$error) {
				...
				if ($old_file) {
					...
					$ext = strtolower(end($arr));
					$manga_image_hash = md5($_FILES["file"]["tmp_name"] . $timestamp) . "." . $ext;
					move_uploaded_file($_FILES["file"]["tmp_name"], ABSPATH . "/images/manga/" . "$manga_image_hash");
				
					$db->query(" UPDATE mangadex_mangas SET manga_image = '$manga_image_hash' WHERE manga_id = $id LIMIT 1; ");
				}
	...
	case "group_edit":
		...
		if (!$error) {
			...
			if ($old_file) {
				$arr = (explode(".", $_FILES["file"]["name"]));
				$ext = strtolower(end($arr));
				$banner_hash = md5($_FILES["file"]["tmp_name"] . $timestamp) . "." . $ext;
				move_uploaded_file($_FILES["file"]["tmp_name"], ABSPATH . "/images/group_banners/$banner_hash");
				
				if ($group->group_banner)
					unlink(ABSPATH . "/images/group_banners/$group->group_banner");
				
				$db->query(" UPDATE mangadex_groups SET group_banner = '$banner_hash' WHERE group_id = $id LIMIT 1; ");
			}
	...
	case "change_profile":
		...
		if (!$error) {
			...
			if ($old_file) {
				$arr = (explode(".", $_FILES["file"]["name"]));
				$ext = strtolower(end($arr));
				$avatar_hash = md5($_FILES["file"]["tmp_name"] . $timestamp) . "." . $ext;
				move_uploaded_file($_FILES["file"]["tmp_name"], ABSPATH . "/images/user_logos/$avatar_hash");
				
				if ($user->avatar)
					unlink(ABSPATH . "/images/user_logos/$user->avatar");
				
				$db->query(" UPDATE mangadex_users SET avatar = '$avatar_hash' WHERE user_id = $user->user_id LIMIT 1; ");
			}
