...
if ($manga->exists) {	
	...
	if ($manga->manga_image) 
		generate_thumbnail(ABSPATH . "/images/manga/$manga->manga_image", 0);
	...
	<div class="panel panel-default">
				...
				<table class="table table-condensed">
					...
					<tr>
						...
						<td><span class="text-info"><?= display_glyphicon("eye", "fas", "Views", "fa-fw") ?> <?= $manga->manga_views ?></span>
							...
							<span><?= display_glyphicon("file", "far", "Total chapters", "fa-fw") ?> <?= $manga->get_total_chapters($db) ?></span></td>
					</tr>
					<tr>
						<th>Description:</th>
		...
		<form class="edit display-none" id="manga_edit_form" method="post" enctype="multipart/form-data">
			<table class="table table-condensed ">
				...
				<tr>
					<th>Author:</th>
					<td><input type="text" class="form-control" id="manga_author" name="manga_author" value="<?= $manga->manga_author ?>" required></td>
				</tr>
				<tr>
					<th>Artist:</th>
	...
	<div class="tab-content">
		<div role="tabpanel" class="tab-pane fade in active" id="chapters">
			...
			require_once(ABSPATH . "/pages/chaptertest.req.php"); 
			...
			<?php
				if ($manga->manga_comments) {
					...
					foreach ($comments as $comment) {
						...
						?>
					<tr class="comment">
						<td width="110px"><img alt="avatar" class="avatar" src="/images/user_logos/<?= $comment_user->logo ?>" /></td>
