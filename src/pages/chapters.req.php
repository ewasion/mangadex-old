		...
		<table class="table table-striped table-hover table-condensed">
			...
			foreach ($chapters as $chapter) {
				...
				<tr id="chapter_<?= $chapter->chapter_id ?>">
					...
					<td><a title="<?= $chapter->title ?>" data-chapter-id="<?= $chapter->chapter_id ?>" data-chapter-num="<?= $chapter->chapter ?>" data-volume-num="<?= $chapter->volume ?>" data-chapter-name="<?= $chapter->title ?>" href="/chapter/<?= $chapter->chapter_id ?>"><?= ($chapter->volume == "" || $chapter->volume == 0) ? "" : "Vol. $chapter->volume " ?><?= ($chapter->chapter != "") ? "Ch. $chapter->chapter - " : "" ?><?= ($chapter->title != "") ? mb_strimwidth($chapter->title, 0, 60, "...") : "Read Online" ?></a></td>
					...
					<td><a style="color: #<?= $chapter->level_colour ?>; " href="/user/<?= $chapter->user_id ?>"><?= $chapter->username ?></a></td>
